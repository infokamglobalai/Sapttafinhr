"""Email one-time passcode for platform login (workspace-controlled 2FA)."""
...
def send_login_otp_email(user, otp: str, *, workspace_name: str = "") -> None:
    label = workspace_name or "Saptta"
...
def issue_and_send_login_otp(user, *, workspace_name: str = "") -> None:
    otp = issue_login_otp(user)
    send_login_otp_email(user, otp, workspace_name=workspace_name)

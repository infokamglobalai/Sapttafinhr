...
class SignupView(APIView):
    ...
    def post(self, request):
        ...
        #    For this deployment, immediately make it ACTIVE without payment.
        _sub_status = Subscription.Status.ACTIVE
        _ent_status = SubscriptionEntitlement.Status.ACTIVE

        # Seed public Module entries
        from .models import Module, CompanyModule
        fin_module, _ = Module.objects.get_or_create(code="FIN", defaults={"name": "Accounting & Finance"})
        hr_module, _ = Module.objects.get_or_create(code="HR", defaults={"name": "HRMS"})
        ...
        # Link plan modules
        if "complete" in plan.code:
            plan.modules.add(fin_module, hr_module)
        elif "hrms" in plan.code:
            plan.modules.add(hr_module)
        elif "finance" in plan.code:
            plan.modules.add(fin_module)
        ...
        # Always grant both FIN and HR so the product switcher shows both.
        _all_products = list(dict.fromkeys(products + [ProductCode.FIN, ProductCode.HR]))
        ...
        # Connect tenant to modules
        for p in products:
            mod = fin_module if p == ProductCode.FIN else hr_module
            CompanyModule.objects.get_or_create(company=tenant, module=mod, defaults={"is_active": True})
        ...
        # Seed dynamic roles/permissions inside tenant schema
        self._seed_tenant_roles(schema_name, products, user)
    ...
    @staticmethod
    def _seed_tenant_roles(schema_name: str, active_products: list[str], owner_user) -> None:
        from apps.team.models import Permission, Role, RolePermission, TenantMember
        ...
        with schema_context(schema_name):
            accounting_perms = [
                ...
                ("masters.manage", "Manage Masters"),
                ("billing.view", "View Billing & Invoices"),
                ("billing.manage", "Manage Billing & Invoices"),
                ("payments.view", "View Payments"),
                ("payments.manage", "Manage Payments"),
                ("procurement.view", "View Purchase & POs"),
                ("procurement.manage", "Manage Purchase & POs"),
                ...
                ("expenses.manage", "Manage Expense Claims"),
            ...
            hrm_perms = [
                ...
                ("performance.view", "View Performance Review"),
                ("performance.manage", "Manage Performance Review"),
                ("recruitment.view", "View Recruitment & Jobs"),
                ("recruitment.manage", "Manage Recruitment & Jobs"),
                ("hr_reports.view", "View HR Reports"),
            ]

            seeded_perms = []
            if ProductCode.FIN in active_products:
                for code, name in accounting_perms:
                    p, _ = Permission.objects.get_or_create(codename=code, defaults={"name": name, "module": "Accounting"})
                    seeded_perms.append(p)
            if ProductCode.HR in active_products:
                for code, name in hrm_perms:
                    p, _ = Permission.objects.get_or_create(codename=code, defaults={"name": name, "module": "HRM"})
                    seeded_perms.append(p)

            # Create default roles
            owner_role, _ = Role.objects.get_or_create(name="Owner", defaults={"description": "Owner with full access", "is_system": True})
            for perm in seeded_perms:
                RolePermission.objects.get_or_create(role=owner_role, permission=perm)

            admin_role, _ = Role.objects.get_or_create(name="Admin", defaults={"description": "Administrator with full access", "is_system": True})
            for perm in seeded_perms:
                RolePermission.objects.get_or_create(role=admin_role, permission=perm)

            if ProductCode.HR in active_products:
                hr_role, _ = Role.objects.get_or_create(name="HR Manager", defaults={"description": "Manages human resources", "is_system": True})
                for perm in seeded_perms:
                    if perm.module == "HRM":
                        RolePermission.objects.get_or_create(role=hr_role, permission=perm)

            if ProductCode.FIN in active_products:
                acct_role, _ = Role.objects.get_or_create(name="Accountant", defaults={"description": "Manages accounting and tax", "is_system": True})
                for perm in seeded_perms:
                    if perm.module == "Accounting":
                        RolePermission.objects.get_or_create(role=acct_role, permission=perm)

                finance_mgr_role, _ = Role.objects.get_or_create(name="Finance Manager", defaults={"description": "Manages financial operations", "is_system": True})
                for perm in seeded_perms:
                    if perm.module == "Accounting":
                        RolePermission.objects.get_or_create(role=finance_mgr_role, permission=perm)

            manager_role, _ = Role.objects.get_or_create(name="Manager", defaults={"description": "Department manager", "is_system": True})
            for perm in seeded_perms:
                if perm.codename in ("ledger.view", "ledger.post", "employees.view", "attendance.view", "leaves.view", "leaves.manage"):
                    RolePermission.objects.get_or_create(role=manager_role, permission=perm)

            supervisor_role, _ = Role.objects.get_or_create(name="Supervisor", defaults={"description": "Team supervisor", "is_system": True})
            for perm in seeded_perms:
                if perm.codename in ("employees.view", "attendance.view", "leaves.view"):
                    RolePermission.objects.get_or_create(role=supervisor_role, permission=perm)

            staff_role, _ = Role.objects.get_or_create(name="Staff", defaults={"description": "General staff member", "is_system": True})
            for perm in seeded_perms:
                if perm.codename in ("employees.view", "attendance.view", "leaves.view") and perm.module == "HRM":
                    RolePermission.objects.get_or_create(role=staff_role, permission=perm)

            # Create TenantMember row for the owner user

          ...
          {items.map((mod, idx) => {
            ...
            <Col key={mod.title} xs={24} md={12} lg={8}>
              <ScrollReveal animation="fade-in-up" delay={idx * 60}>
                <article className={`marketing-module-card${featured ? ' marketing-module-card--featured' : ''}`} style={{ borderTopColor: mod.accent }}>
                  <div className="marketing-module-card__top">
                    <span className="marketing-module-card__code" style={{ color: mod.accent }}>{mod.code}</span>
                    <Tag style={{ borderRadius: 8, fontWeight: 700 }}>{mod.tag}</Tag>
                  </div>

...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        if (cancelled) return;
        localStorage.setItem('saptta_products', JSON.stringify(products ?? []));
  ...
  const loginDemo = async () => {
    ...
    try {
      ...
      localStorage.setItem('saptta_products', JSON.stringify(products ?? ['finance', 'hrms']));
      setUser(toAppUser(me, products ?? ['finance', 'hrms'], 'demo'));
      setToken(res.access);
    } finally {
      setIsLoading(false);
    }
  ...
  const signup = async (data: SignupData) => {
    ...
    try {
      ...
      const finalProducts = result.products ?? productsForPlan(data.planId);
      localStorage.setItem('saptta_products', JSON.stringify(finalProducts));
      setUser(
        toAppUser(

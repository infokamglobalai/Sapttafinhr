...
const getApiBaseUrl = () => {
  if (isElectron) {
    const saved = localStorage.getItem('saptta_backend_url');
    if (saved) return saved;
    return import.meta.env.VITE_PLATFORM_API_BASE_URL || 'https://app.saptta.com/api/v1';
  }
  return import.meta.env.VITE_PLATFORM_API_BASE_URL || 'http://localhost:8000/api/v1';
};

const PLATFORM_BASE = getApiBaseUrl();
...
async function rawRequest<T>(path: string, opts: RequestOptions, accessToken: string | null): Promise<T> {
  ...
  if (surface === 'tenant' || workspace || getWorkspace()) {
    const ws = workspace || getWorkspace();
    if (ws) {
      finalHeaders['X-Tenant'] = ws;
    }
  }
...
export async function demoLogin(): Promise<{ access: string; refresh: string; workspace?: string | null }> {
  const data = await rawRequest<{ access: string; refresh: string; workspace?: string | null }>(
    '/auth/demo-login/',
    { surface: 'platform', method: 'POST', auth: false },
    null,
  );
  setTokens(data.access, data.refresh);
  setWorkspace('demo');

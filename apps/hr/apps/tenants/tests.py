...
class DemoModeAndLockTest(TestCase):
    """Verifies that the demo account is locked down and seeding runs correctly."""

    def test_demo_user_password_lock(self):
        # 1. Create a demo user
        from apps.accounts.models import User
        from apps.tenants.models import Tenant
        tenant = Tenant.objects.create(name="Demo Co", subdomain="demotest", status="active")
        ...
        self.assertEqual(demo.email, "demo@saptta.com")
        ...
        with self.assertRaises(ValidationError) as excinfo:
            demo.save()
        self.assertIn("Password changes are disabled for the demo account", str(excinfo.exception))

    def test_normal_user_password_change_succeeds(self):
        # A normal user should be allowed to change password
        from apps.accounts.models import User
        from apps.tenants.models import Tenant
        tenant = Tenant.objects.create(name="Normal Co", subdomain="normaltest", status="active")
        
        user = User.objects.create_user(
            email="normal@saptta.com",
            tenant=tenant,
    ...
    def test_demo_tenant_and_user_seeded(self):
        # The post_migrate signal should have automatically run the seeding
        # of the HR demo tenant and demo@saptta.com user during test DB setup.
        from apps.tenants.models import Tenant, ProductEntitlement
        from apps.accounts.models import User, Role, UserRole

        tenant = Tenant.objects.filter(subdomain="demo").first()
        self.assertIsNotNone(tenant, "HR Demo tenant should be automatically seeded.")
        self.assertEqual(tenant.status, "trial")
        self.assertTrue(tenant.setup_complete)

        user = User.objects.filter(email="demo@saptta.com", tenant=tenant).first()
        self.assertIsNotNone(user, "HR Demo user should be automatically seeded.")
        self.assertTrue(user.is_active)

        # Verify role assignment
        super_admin_role = Role.objects.filter(tenant=tenant, name="super_admin").first()
        self.assertIsNotNone(super_admin_role)
        self.assertTrue(UserRole.objects.filter(user=user, role=super_admin_role).exists())

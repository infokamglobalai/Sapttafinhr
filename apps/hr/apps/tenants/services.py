...
def get_tenant_email_connection(tenant):
    """
    Get a custom django.core.mail connection configured with the tenant's SMTP settings.
    Returns (connection, from_email). If no custom settings are configured, returns (None, default_from_email).
    """
    ...
    host = TenantSetting.get(tenant, "SMTP_HOST", "").strip()
    port_str = TenantSetting.get(tenant, "SMTP_PORT", "").strip()
    user = TenantSetting.get(tenant, "SMTP_USER", "").strip()
    password = TenantSetting.get(tenant, "SMTP_PASSWORD", "").strip()
    use_tls = TenantSetting.get(tenant, "SMTP_USE_TLS", "True").strip().lower() == "true"
    from_email = TenantSetting.get(tenant, "SMTP_FROM_EMAIL", "").strip()

    if host and port_str and user and password:
        try:
            port = int(port_str)
        except ValueError:
            port = 587
        ...
        return connection, from_email or user

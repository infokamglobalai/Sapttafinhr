...
@login_required
@require_POST
def payroll_run_disburse(request, pk):
    """
    Connected Banking API Payout: processes a real bulk IMPS/NEFT transfer 
    of salaries for all approved employee accounts via ICICI or HDFC Connected Banking.
    """
    import os
    tenant = request.tenant
    run = get_object_or_404(PayrollRun, pk=pk, tenant=tenant)
    if run.status != "approved":
        messages.error(request, "Only approved runs can be paid out via Connected Banking.")
        return redirect("payroll:run_detail", pk=pk)

    bank_choice = request.POST.get("bank_account")
    if not bank_choice:
        messages.error(request, "Please select a corporate bank account to disburse from.")
        return redirect("payroll:run_detail", pk=pk)

    records = run.records.select_related("employee").prefetch_related("employee__bank_accounts")

    client = None
    source_account = None

    import sys
    fin_path = "c:/Users/user/Downloads/saptta/saptta_finhr/apps/finance/backend"
    if fin_path not in sys.path:
        sys.path.append(fin_path)
    from apps.banking.models import BankAccount
    from apps.banking.services import decrypt_credentials
    from django.db.models import Q

    bank_keyword = ""
    if "icici" in bank_choice.lower():
        bank_keyword = "icici"
    elif "hdfc" in bank_choice.lower():
        bank_keyword = "hdfc"
    elif "axis" in bank_choice.lower():
        bank_keyword = "axis"
    elif "sbi" in bank_choice.lower():
        bank_keyword = "sbi"

    db_creds = {}
    if bank_keyword:
        q = Q(bank_name__icontains=bank_keyword)
        if bank_keyword == "sbi":
            q = q | Q(bank_name__icontains="state bank")
        bank_account = BankAccount.objects.filter(q).first()
        if bank_account and hasattr(bank_account, "credential") and bank_account.credential.encrypted_data:
            try:
                db_creds = decrypt_credentials(bank_account.credential.encrypted_data)
            except Exception:
                pass
    ...
    if "icici" in bank_choice.lower():
        ...
        source_account = db_creds.get("corp_account") or os.environ.get("ICICI_CORP_ACCOUNT")
        ...
        missing = [k for k, v in [
            ...
            ("ICICI_CORP_ACCOUNT", source_account)
        ] if not v]

        if missing:
            messages.error(request, f"Please configure missing ICICI connected banking credentials in your .env or DB: {', '.join(missing)}")
            return redirect("payroll:run_detail", pk=pk)

        from apps.banking.integrations.icici import ICICIConnectedBankingClient
        try:
            ...
            messages.error(request, f"Failed to initialize ICICI API client: {client_init_err}")
            return redirect("payroll:run_detail", pk=pk)
        ...
        source_account = db_creds.get("corp_account") or os.environ.get("HDFC_CORP_ACCOUNT")
        ...
        missing = [k for k, v in [
            ...
            ("HDFC_CORP_ACCOUNT", source_account)
        ] if not v]

        if missing:
            messages.error(request, f"Please configure missing HDFC connected banking credentials in your .env or DB: {', '.join(missing)}")
            return redirect("payroll:run_detail", pk=pk)

        from apps.banking.integrations.hdfc import HDFCConnectedBankingClient
        ...
        source_account = db_creds.get("corp_account") or os.environ.get("AXIS_CORP_ACCOUNT")
        ...
        missing = [k for k, v in [
            ...
            ("AXIS_CORP_ACCOUNT", source_account)
        ] if not v]

        if missing:
            messages.error(request, f"Please configure missing Axis connected banking credentials in your .env or DB: {', '.join(missing)}")
            return redirect("payroll:run_detail", pk=pk)

        from apps.banking.integrations.axis import AxisConnectedBankingClient
        try:
            ...
            messages.error(request, f"Failed to initialize Axis API client: {client_init_err}")
            return redirect("payroll:run_detail", pk=pk)
        ...
        source_account = db_creds.get("corp_account") or os.environ.get("SBI_CORP_ACCOUNT")
        ...
        missing = [k for k, v in [
            ...
            ("SBI_CORP_ACCOUNT", source_account)
        ] if not v]

        if missing:
            messages.error(request, f"Please configure missing SBI connected banking credentials in your .env or DB: {', '.join(missing)}")
            return redirect("payroll:run_detail", pk=pk)

        from apps.banking.integrations.sbi import SBIConnectedBankingClient
        client = SBIConnectedBankingClient(
            corp_id=corp_id, user_id=user_id, client_id=client_id, client_secret=client_secret
        )

    else:
        messages.error(request, "Real API payouts are only supported for ICICI, HDFC, Axis, and SBI corporate accounts.")
        return redirect("payroll:run_detail", pk=pk)
    ...
    for rec in records:
        ...
        try:
            client.initiate_payout(
                ...
                beneficiary_ifsc=bank.ifsc_code or bank.ifsc,  # Support both field variations
                amount=float(rec.net_payable),
                narration=f"Sal {run.year}-{run.month:02d}"
            )
            payout_count += 1
            payout_amount += rec.net_payable
        except Exception as payout_err:
            failed_payouts.append(f"{emp.full_name} ({payout_err})")

    if failed_payouts:
        messages.error(request, f"Connected Banking API transfer failed for: {', '.join(failed_payouts)}")
        return redirect("payroll:run_detail", pk=pk)

    # Transition status to paid only on absolute success
    run.status = "paid"
    ...
    try:
        from apps.hr_ops.services import audit_log
        audit_log(
            tenant, request.user, "disburse_api", "PayrollRun", run,
            f"API bulk disbursement of ₹{payout_amount:,.2f} complete via {bank_choice}.",
            details={
                "bank": bank_choice,
    ...
    msg = f"One-click connected banking payout executed! Successful payouts: {payout_count}. Total disbursed: ₹{payout_amount:,.2f}."
    if skipped:
        msg += f" Skipped {len(skipped)} employees lacking bank accounts: {', '.join(skipped)}."
